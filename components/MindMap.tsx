...
const MindMapCategory: React.FC<{ title: string; items: string[]; onNodeClick: (query: string) => void, color: string }> = ({ title, items, onNodeClick, color }) => (
    ...
    <div className="flex flex-col items-center gap-2">
      {items.map(item => (
        <button
          ...
          className="w-full text-center px-3 py-2 bg-gray-700/60 rounded-md hover:bg-gray-700 text-gray-300 transition-colors capitalize"
        >
          {item}
        </button>
      ))}
...
const MindMap: React.FC<MindMapProps> = ({ centerNode, data, onNodeClick }) => {
  return (
    <div className="animate-fade-in space-y-6 w-full max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 text-sm">
        <MindMapCategory title="Foundational" items={data.foundational} onNodeClick={onNodeClick} color="border-gray-500" />
        <MindMapCategory title="Deeper Dive" items={data.deeperDive} onNodeClick={onNodeClick} color="border-gray-500" />
        <MindMapCategory title="Related Branches" items={data.relatedBranches} onNodeClick={onNodeClick} color="border-gray-500" />
      </div>
    </div>
  );
};
